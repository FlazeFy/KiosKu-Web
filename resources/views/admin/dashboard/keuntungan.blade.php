@if(session()->get('view_keuntungan_Key') == null)
    @php(session()->put('view_keuntungan_Key', '1'))
@endif
...
<div class="container-fluid p-3 rounded shadow h-100">
    ...
    <div class="dropdown-menu dropdown-menu-end" aria-labelledby="cardOpt-keuntungan-view">
        <form action="/dashboard/filter/keuntungan" method="POST">
            @csrf
            <button type="submit" class="dropdown-item <?php if(session()->get('view_keuntungan_Key') == 1){echo "active";} ?>" name="view" value="1">Mingguan</button>
        </form>
        <form action="/dashboard/filter/keuntungan" method="POST">
            @csrf
            <button type="submit" class="dropdown-item <?php if(session()->get('view_keuntungan_Key') == 2){echo "active";} ?>" name="view" value="2">Bulanan</button>
        </form>
    </div>
    <a class="float-start title">Keuntungan</a><br>
    <h5 class="price mt-3">Rp. 
        @php($count = 0)
        @foreach($transaksi as $ts)
            @foreach($barang_transaksi as $btrs)
                @if(($btrs->id_keranjang == $ts->id)&&(strtotime($ts->created_at) > strtotime('-7 day'))&&(session()->get('view_keuntungan_Key') == '1'))
                    @php($count += $btrs->qty * ($btrs->harga_jual - $btrs->harga_stok))
                @elseif (($btrs->id_keranjang == $ts->id)&&(strtotime($ts->created_at) > strtotime('-30 day'))&&(session()->get('view_keuntungan_Key') == '2'))
                    @php($count += $btrs->qty * ($btrs->harga_jual - $btrs->harga_stok))
                @endif
            @endforeach
        @endforeach
